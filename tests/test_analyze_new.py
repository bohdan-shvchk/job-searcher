...
class TestIsAggregateUrl(unittest.TestCase):
    def test_linkedin_is_aggregate(self):
        self.assertTrue(analyze_new.is_aggregate_url("https://linkedin.com/jobs/search?q=webflow"))
    ...
    def test_specific_djinni_job_is_not_aggregate(self):
        self.assertFalse(analyze_new.is_aggregate_url("https://djinni.co/jobs/123456-webflow-dev/"))
    ...
    def test_dou_search_is_aggregate(self):
        self.assertTrue(analyze_new.is_aggregate_url("https://jobs.dou.ua/vacancies/?search=Webflow"))
...
class TestTextExtractor(unittest.TestCase):
    ...
    def test_skips_nav_and_footer(self):
        parser = analyze_new.TextExtractor()
        parser.feed("<p>Main</p><nav>Menu</nav><footer>Footer</footer>")
        text = parser.get_text()
