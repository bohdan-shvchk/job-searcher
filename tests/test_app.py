...
# Point DATA_DIR to a temp directory before importing app
...
class TestAppEndpoints(unittest.TestCase):
    ...
    def test_api_last_check_returns_json(self):
        resp = self.client.get("/api/last-check")
        self.assertEqual(resp.status_code, 200)
        data = json.loads(resp.data)
    ...
    def test_api_statuses_returns_dict(self):
        resp = self.client.get("/api/statuses")
        self.assertEqual(resp.status_code, 200)
        data = json.loads(resp.data)
    ...
    def test_api_export_returns_csv(self):
...
class TestParseVacancies(unittest.TestCase):
    ...
    def test_vacancy_has_status_field(self):
        with open(self.md_path, "w", encoding="utf-8") as f:
            f.write("## Test\n- [Job](https://example.com/1)\n")
        result = app_module.parse_vacancies()
        item = result[0]["items"][0]
        self.assertIn("vacancy_status", item)
        self.assertEqual(item["vacancy_status"], "new")

    def test_vacancy_has_comment_field(self):
        with open(self.md_path, "w", encoding="utf-8") as f:
            f.write("## Test\n- [Job](https://example.com/1)\n")
        result = app_module.parse_vacancies()
        item = result[0]["items"][0]
